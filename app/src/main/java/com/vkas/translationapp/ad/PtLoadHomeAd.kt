...
class PtLoadHomeAd {
    ...
    fun advertisementLoadingPt(context: Context) {
        ...
        KLog.d(logTagPt, "home--isLoading=${isLoadingPt}")

        if (isLoadingPt) {
            KLog.d(logTagPt, "home--广告加载中，不能再次加载")
            return
        }
        if (appAdDataPt == null) {
            isLoadingPt = true
    ...
    private fun loadHomeAdvertisementPt(context: Context, adData: PtAdBean) {
        val id = takeSortedAdIDPt(adIndexPt, adData.pt_home)
        KLog.d(logTagPt, "home---原生广告id=$id;权重=${adData.pt_home.getOrNull(adIndexPt)?.pt_weight}")
        ...
        homeNativeAds.withAdListener(object : AdListener() {
          ...
          """"
                isLoadingPt = false
                appAdDataPt = null
                KLog.d(logTagPt, "home---加载home原生加载失败: $error")

                if (adIndexPt < adData.pt_home.size - 1) {
                    adIndexPt++
            ...
            override fun onAdLoaded() {
                super.onAdLoaded()
                KLog.d(logTagPt, "home---加载home原生广告成功")
            ...
            override fun onAdOpened() {
                super.onAdOpened()
                KLog.d(logTagPt, "home---点击home原生广告")
     ...
     * 设置展示home原生广告
     */
    fun setDisplayHomeNativeAdPt(activity: AppCompatActivity, binding: ActivityMainBinding) {
