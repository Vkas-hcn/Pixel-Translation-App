...
class PtLoadConnectAd {
    companion object {
        fun getInstance() = InstanceHelper.openLoadPt
    }

    object InstanceHelper {
        val openLoadPt = PtLoadConnectAd()
    }
    ...
    fun advertisementLoadingPt(context: Context) {
        ...
        KLog.d(logTagPt, "connect--isLoading=${isLoadingPt}")

        if (isLoadingPt) {
            KLog.d(logTagPt, "connect--广告加载中，不能再次加载")
            return
        }

        if (appAdDataPt == null) {
            isLoadingPt = true
            loadConnectAdvertisementPt(context, getAdServerDataPt())
        }
     ...
     * 加载首页插屏广告
     */
    private fun loadConnectAdvertisementPt(context: Context, adData: PtAdBean) {
        val adRequest = AdRequest.Builder().build()
        val id = takeSortedAdIDPt(adIndexPt, adData.pt_connect)
        KLog.d(
            logTagPt,
            "connect--插屏广告id=$id;权重=${adData.pt_connect.getOrNull(adIndexPt)?.pt_weight}"
        )
        ...
        InterstitialAd.load(
            ...
            object : InterstitialAdLoadCallback() {
                override fun onAdFailedToLoad(adError: LoadAdError) {
                    adError.toString().let { KLog.d(logTagPt, "connect---连接插屏加载失败=$it") }
                    isLoadingPt = false
                    appAdDataPt = null
                    if (adIndexPt < adData.pt_connect.size - 1) {
                        adIndexPt++
                        loadConnectAdvertisementPt(context, adData)
                    } else {
                        adIndexPt = 0
                    }
                }
                ...
                override fun onAdLoaded(interstitialAd: InterstitialAd) {
                    ...
                    adIndexPt = 0
                    KLog.d(logTagPt, "connect---连接插屏加载成功")
                }
            })
    }
        ...
        appAdDataPt?.fullScreenContentCallback =
            object : FullScreenContentCallback() {
                override fun onAdClicked() {
                    ...
                    KLog.d(logTagPt, "connect插屏广告点击")
                    recordNumberOfAdClickPt()
                }

                override fun onAdDismissedFullScreenContent() {
                    // Called when ad is dismissed.
                        KLog.d(logTagPt, "关闭connect插屏广告=${App.isBackDataPt}")
                        LiveEventBus.get<Boolean>(Constant.PLUG_PT_ADVERTISEMENT_SHOW)
     ...
     * 展示Connect广告
     */
    fun displayConnectAdvertisementPt(activity: AppCompatActivity): Boolean {
        if (appAdDataPt == null) {
            KLog.d(logTagPt, "connect--插屏广告加载中。。。")
            return false
        }
        if (whetherToShowPt || activity.lifecycle.currentState != Lifecycle.State.RESUMED) {
            KLog.d(logTagPt, "connect--前一个插屏广告展示中或者生命周期不对")
            return false
        }
